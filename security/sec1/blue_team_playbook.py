...
try:
    ...
    from sec1.red_team_toolkit import RedTeamTestResult, VulnerabilityClass
    ...
    try:
        from sec1.red_team_toolkit import RedTeamTestResult
    except ImportError:
...
class BlueTeamPlaybook:
    ...
    def _extract_vm_id(self, target_id: str) -> Optional[UUID]:
        """Helper to safely extract a VM UUID from a target ID string with enhanced parsing."""
        try:
            # Try direct UUID parsing first
            return UUID(target_id)
        except (ValueError, TypeError):
            # Enhanced parsing for complex target identifiers
            try:
                # Handle URLs and extract UUIDs from paths
                if target_id.startswith(('http://', 'https://')):
                    from urllib.parse import urlparse
                    parsed = urlparse(target_id)
                    # Extract UUID from path components
                    path_parts = parsed.path.split('/')
                    for part in path_parts:
                        if len(part) == 36 and part.count('-') == 4:  # UUID format
                            return UUID(part)
                
                # Handle VM names by looking up in supervisor
                if hasattr(self, 'vm_supervisor') and self.vm_supervisor:
                    try:
                        vms = self.vm_supervisor.list_vms()
                        for vm in vms:
                            if hasattr(vm, 'name') and vm.name == target_id:
                                return UUID(vm.vm_id)
                    except Exception as e:
                        logger.debug(f"Failed to lookup VM by name: {e}")
                
                # Try to extract UUID patterns from the string
                import re
                uuid_pattern = r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'
                match = re.search(uuid_pattern, target_id, re.IGNORECASE)
                if match:
                    return UUID(match.group())
                
            except Exception as e:
                logger.debug(f"Enhanced UUID extraction failed: {e}")
            
            # Final fallback: generate a deterministic UUID based on the target_id
            # This ensures consistent behavior for the same target
            import hashlib
            hash_input = target_id.encode('utf-8')
            hash_digest = hashlib.md5(hash_input).hexdigest()
            uuid_str = f"{hash_digest[:8]}-{hash_digest[8:12]}-{hash_digest[12:16]}-{hash_digest[16:20]}-{hash_digest[20:32]}"
            logger.info(f"Generated deterministic UUID {uuid_str} for target_id: {target_id}")
    ...
    def _create_playbook_event_for_manual_review(self, threat_result) -> Any:
        """Creates a comprehensive SecurityEvent for threats that require manual review."""
        try:
            from network_security import SecurityEvent
        except ImportError:
            class SecurityEvent:
                ...
                def to_dict(self):
                    return {
                        ...
                        "severity": getattr(self, 'threat_level', 'unknown'),
                        "created_at": self.created_at.isoformat()
                    }
        
        # Enhanced details with comprehensive threat context
        details = {
            ...
            "discovered_at": threat_result.timestamp.isoformat() if threat_result.timestamp else datetime.utcnow().isoformat(),
            "test_type": threat_result.test_type.value if hasattr(threat_result, 'test_type') else "unknown",
            "execution_time_ms": getattr(threat_result, 'execution_time_ms', 0),
            "mitigation_suggestions": getattr(threat_result, 'mitigation_suggestions', []),
            "metadata": getattr(threat_result, 'metadata', {}),
            "review_priority": self._calculate_review_priority(threat_result),
            "recommended_actions": self._generate_recommended_actions(threat_result),
            "escalation_required": self._requires_escalation(threat_result)
        }
        ...
        return SecurityEvent(
            ...
            timestamp=threat_result.timestamp if threat_result.timestamp else datetime.utcnow(),
            details=details
        )
    
    def _calculate_review_priority(self, threat_result) -> str:
        """Calculate the priority for manual review based on threat characteristics."""
        severity = getattr(threat_result, 'severity', ThreatLevel.MEDIUM)
        vuln_class = getattr(threat_result, 'vulnerability_class', None)
        
        # Critical vulnerabilities always get highest priority
        ...
        if vuln_class in high_impact_classes and severity == ThreatLevel.HIGH:
            return "HIGH"
    ...
    def _generate_recommended_actions(self, threat_result) -> List[str]:
        """Generate recommended actions for manual review."""
        actions = []
        vuln_class = getattr(threat_result, 'vulnerability_class', None)
        
        # Base actions for all vulnerabilities
        actions.extend([
            "Verify the vulnerability through independent testing",
        ...
        if vuln_class == VulnerabilityClass.INJECTION:
            actions.extend([
                ...
                "Check for similar vulnerabilities in related systems",
                "Implement immediate input sanitization"
            ])
        elif vuln_class == VulnerabilityClass.BROKEN_ACCESS:
            actions.extend([
                "Review access control policies",
                "Audit user permissions and roles",
                "Check for privilege escalation paths"
            ])
        elif vuln_class == VulnerabilityClass.SECURITY_MISCONFIG:
            actions.extend([
    ...
    def _requires_escalation(self, threat_result) -> bool:
        """Determine if the threat requires escalation to senior security team."""
        severity = getattr(threat_result, 'severity', ThreatLevel.MEDIUM)
        vuln_class = getattr(threat_result, 'vulnerability_class', None)
        
        # Always escalate critical vulnerabilities
        if severity == ThreatLevel.CRITICAL:
            return True
        
        # Escalate high-impact vulnerabilities
        critical_classes = [
            VulnerabilityClass.INJECTION,
            VulnerabilityClass.BROKEN_ACCESS,
            VulnerabilityClass.KNOWN_VULNERABILITIES
        ...
        if vuln_class in critical_classes and severity == ThreatLevel.HIGH:
            return True
